...
class GameScene extends Phaser.Scene {
  ...
  create() {
  // Fond gradient néon + étoiles/particules légères
  ...
  // Grille d’arène cyberpunk (lignes fines néon)
  ...
  // Particules de fond (glow flottant)
  this.add.particles(0, 0, null, {  // pas d'image, on utilise des cercles
    speed: { min: 10, max: 30 },
    scale: { start: 0.8, end: 0 },
    blendMode: 'ADD',
    lifespan: 3000,
    frequency: 200,
    tint: [0x00ffff, 0xff00ff, 0x00ffaa],
    alpha: { start: 0.8, end: 0 }
  });
  
    // Noyau joueur (cyan, pulsation + glow multiple)
const playerCore = this.add.circle(150, this.scale.height / 2, 60, 0x00ffff);
playerCore.setStrokeStyle(12, 0x00ffff, 0.8);
this.add.circle(150, this.scale.height / 2, 70, 0x00ffff, 0.3); // glow externe
this.add.circle(150, this.scale.height / 2, 90, 0x00ffff, 0.1); // glow externe 2
...
// Même chose pour noyau ennemi (magenta)
...
this.tweens.add({
  ...
  ease: 'Sine.easeInOut'
    ...
    // UI énergie (barre néon)
this.energy = 0;
this.energyBar = this.add.rectangle(20, 20, 200, 40, 0x00ffff, 0.3).setOrigin(0);
this.energyFill = this.add.rectangle(20, 20, 0, 40, 0x00ffff).setOrigin(0, 0.5);
this.energyText = this.add.text(120, 20, 'ÉNERGIE 0/10', {
  fontSize: '24px',
  color: '#ffffff',
  stroke: '#00ffff',
  strokeThickness: 3
